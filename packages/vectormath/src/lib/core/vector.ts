...
export class Vector extends Array<number> {
  ...
  add(vector: Vector | number[] | number): Vector {
    if (vector instanceof Vector || vector instanceof Array) {
      ...
      throw new Error(
        `Parameter must be of type Vector, Array<number> or number`
      );
    }
  }
  ...
  angle(vector: Vector): number {
    return Math.acos(
      this.scalarProduct(vector) / (this.distance * vector.distance)
    );
  }
   ...
   * @param vector
   */
  dot<T extends Vector | number[] | number>(
    vector: T
  ): T extends Vector ? number : Vector;
  ...
  cross(vector: Vector | number[]): Vector {
    ...
    if (this.length !== vector.length) {
      throw new Error(
        `Can't build the vector-product of unequally sized vectors`
      );
    }
