...
describe('matrix', () => {
  describe('instantiation and filling', () => {
    ...
    it('should throw an error if one of the rows was transformed manually', () => {
      ...
      expect(() => {
        ...
        new RegExp(
          `Your matrix is broken, it contains rows with different dimensions: 3 and 2`
        )
      );
    });
  ...
  describe('determinant', () => {
    ...
    it('should throw an error if matrix is not NxN shaped', () => {
      ...
      expect(() => {
        matrix.determinant;
      }).toThrow(new RegExp(`Determinants require matrices having shape NxN`));
    });
  });
  ...
  describe('trace', () => {
    ...
    it('should throw an error if matrix is not NxN shaped', () => {
      ...
      expect(() => {
        matrix.trace;
      }).toThrow(new RegExp('Trace requires a NxN matrix'));
    });
  });
  ...
  describe('extract', () => {
    ...
    it('should throw an error when indices out of bounds', () => {
      ...
      expect(() => {
        matrix.extract([1, 2], [2, 3]);
      }).toThrow(new Error(`Invalid range: [1,2] [2,3] for shape [2,2]`));
    });
  });
    ...
    describe('vector', () => {
      ...
      it('should throw an error if vector has wrong dimensions', () => {
        ...
        expect(() => {
          ...
          new RegExp(
            `Matrix-vector multiplication requires the dimensions to be`
          )
        );
      });
  ...
  describe('solve', () => {
    ...
    it('should solve the test 2d linear system of equations', () => {
      ...
      if (solution) {
        const error = [
          Math.abs(solution[0] - 0.55),
          Math.abs(solution[1] + 0.15),
        ];
