 ...
 * @param segment1
 */
export function intersection(segment0: [Vector, Vector], segment1: [Vector, Vector]): Vector | null {
  ...
  if (!isTwoDimensional([...segment0, ...segment1])) {
    throw new Error(`To compute the 2 dimensional intersection, 2 dimensional vectors are required`);
  }
...
function fixNegativeZero(vector: Vector): Vector {
  return new Vector().from(vector.map(o => (o === -0 ? 0 : o)));
}
...
function intersectionOnSegement(point: Vector, segment: [Vector, Vector]) {
  ...
  if (
    ...
    (product < 0 || product > squaredLength) ||
